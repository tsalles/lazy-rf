...
cur_path=`pwd`;
svm_path="/home/tsalles/Programacao/liblinear-1.94";
...
#  $svm_scale           -r ${output}/scale $teste  > ${output}/tmp; mv ${output}/tmp $teste ;
  ...
  local C=0; local G=0;
  cd $svm_path/tools;
    ./grid.py -log2c -5,15,2 -log2g 1,1,1 -svmtrain $svm_train -e 0.001 $treino > $output/saidaGrid 2>/dev/null;
    C=`cat ${output}/saidaGrid | tail -n 1 | awk '{print $1;}'`;
#    G=`cat ${cur_path}/saidaGrid | tail -n 1 | awk '{print $2;}'`;
  ...
  ./createAffMatrix.pl $teste ${output}/predict_svm $round >> ${output}/ResSVM_BL;
...
#  time ./tcpp -d ${output}/treino -t ${output}/teste -m knn -k 3 -D l2 -R >> ${output}/ResKNN;
#
#  echo "#$i" >> ${output}/ResNB;
...
# ./tsalles2libsvm ${output}/treino  > ${output}/treino_svm;
# ./tsalles2libsvm ${output}/teste   > ${output}/teste_svm; 
# time svm_classify ${output}/treino_svm ${output}/teste_svm $i;

#  # discrete data
#  echo "#$i" >> ${output}/ResCDT;
#  time ./tcpp -d ${output}/treino -t ${output}/teste -m cdt -r $i >> ${output}/ResCDT 2> log;
#
#echo "#$i" >> ${output}/ResRF;
#time ./tcpp -d ${output}/treino -t ${output}/teste -m crf -r $i -x 0.5 -n 200 >> ${output}/ResRF;
#
#echo "#$i" >> ${output}/ResLRF;
#time ./tcpp -d ${output}/treino -t ${output}/teste -m crf-knn -r $i -x 0.1 -k 5 -D l2 -n 100 >> ${output}/ResLRF;
#
#echo "#$i" >> ${output}/ResBRF;
#time ./tcpp -d ${output}/treino -t ${output}/teste -m crf-boosted -r $i -x 0.000001 -Z -n 50 >> ${output}/ResBRF;
