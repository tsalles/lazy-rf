...
#include "rf_bst.hpp"
#include "rf_cont.hpp"
#include "rf_knn_cont.hpp"
#include "rf_bst_cont.hpp"
...
method_t parse_method(const char* m) {
  ...
  else if (strcmp(m, CDT_STR) == 0) return CNT_DT;
  ...
  else if (strcmp(m, CRF_STR) == 0) return CNT_RF;
  else if (strcmp(m, CRFKNN_STR) == 0) return CNT_RFKNN;
  else if (strcmp(m, CBRF_STR) == 0) return CNT_BRF;
  else {
...
SupervisedClassifier * get_traditional_classifier(method_t &m, params_t &p){
 ...
 switch(m) {
  ...
  case BRF:
     ...
     break;
  case CNT_RF:
     cc = new CRF(p.round, p.rf_m, p.rf_trees, p.rf_height);
     break;
  case CNT_RFKNN:
     cc = new CRF_KNN(p.round, p.rf_m, p.k, p.rf_trees);
     break;
  case CNT_BRF:
     cc = new CRF_BOOST(p.round, p.rf_m, p.rf_trees, p.rf_height, p.rf_trn);
     break;
  default:
     std::stringstream ss;
